...
This function should only modify configuration layer settings."
  (setq-default
   ...
   '(
     ...
     treemacs
     osx
     ;; themes-megapack ; this caused font size not to load with emacs --daemon
     )
...
It should only modify the values of Spacemacs settings."
  ...
  (setq-default
   ...
   dotspacemacs-themes '(
                         (solarized-selenized-black :location (recipe :fetcher github
                                                                      :repo "bbatsov/solarized-emacs"))
                         (modus-vivendi :location (recipe :fetcher github
                         ...
                         professional
                         leuven
                         (zonokai-red :location (recipe :fetcher github
   ...
   ;; (default nil)
   ;; SPC t n n doesn't toggle on line number if this is set to nil
   ;; but if set to t, line number are on by default. No perfect configuration.
...
before packages are loaded."
  ;; The following code uses tcp server when a given server name is detected
  ;; must put here instead of user-init in order to get the updated `server-name'
  ;; (when (server-running-p "tcp_server")
  ;; (setq-default server-use-tcp t)
  ;; (setq-default server-auth-dir "/tmp/emacs/") ; dotspacemacs-server-socket-dir is only for socket
  ;; (setq-default server-port 9999)
  ...
  (spacemacs/set-leader-keys
    ...
    "q F" 'spacemacs/kill-emacs)   ; force quit, lose all changes
  (spacemacs/set-leader-keys
  ...
  (global-set-key "\e[1;9D" (kbd "H-S-<left>"))   ; ^[[1;9D mapped to cmd-shift-left
  ...
  (with-eval-after-load 'company
    ...
    (define-key evil-insert-state-map (kbd "S-SPC") 'company-manual-begin) ;; Shift-SPC
    )
  ...
  (cond
   ((string-equal system-type "darwin")
    (progn (setq TeX-view-program-selection '((output-pdf "displayline")))))
   )
  )
